...
const ItemList = () => {
  ...
  useEffect(() => {
    ...
    // para sa checkbox na ang selectall ray macheckan
...
// this code will back all tthe checkbox to unchecked
  ...
  const handleDeleteAll = async() => {
    ...
    if(result.isConfirmed){
      try {
        const response = await axios.delete(`/api/item`, {data : {ids : arrayCheck}})
  ...
  const [categoryList, setCategoryList] = useState<Array<CATEGORY>>([])
    ...
    const getCategory = async() => {
        try {
            const response = await axios.get('/api/item/create')
            console.log("jkasdhfklhsk,hkssaljdlkghasjf",response.data.data)
  ...
  const [itemPagination, setItemPagination] = useState<Array<PAGINATION>>([])
  ...
  const handleFetchData = async(url : string | null, search : string | null) => {
          try {
            if(!url){
              ...
              setItemDataList(response.data.data.data)
              ...
              console.log(`/${urlSplit[urlSplit.length-2]}/${urlSplit[urlSplit.length-1]}`)
              setItemDataList(response.data.data.data)
              console.log("this is the pager",response.data.data.links)
              setItemPagination(response.data.data.links)
              console.log("this is the countperPage",response.data.data.per_page)
  ...
  const handleCategoryFind = (category_id : number) => {
    const category = categoryList.find(category => category.id === category_id);
    if (category) {
  ...
  return (
    <DefaultLayout>
        ...
        <div className={`fixed inset-0 flex justify-center items-center z-[1000] bg-black/10 backdrop-blur-[2px] ${isOpenModal ? 'visible' : 'invisible'} transition-all duration-200 delay-75`} onClick={handleModal}>
          <ModalItem clickOn={(e) => e.stopPropagation()} isOpenModal={isOpenModal} handleModal={handleModal} editId={editId} itemDataList={itemDataList} categoryList={categoryList} handleFetchData={()=> handleFetchData(null,'')}/>
        </div>
            ...
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg py-4 px-2 mb-6 bg-black/20 dark:bg-white/20">
                <table className="w-full text-sm text-left rtl:text-right">
                    ...
                    <tbody className= 'text-black font-semibold'>
                      {!isLoading ?
                        (itemDataList.length !== 0 ? 
                          (itemDataList?.map((item : ITEMDATABASE ,index : number) => (
                            <tr key={index} className="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 even:bg-blue-100 even:border-y-[1px] even:border-blue-700">
                                ...
                                <td className="px-6 py-4">
                                    <img src={`http://localhost:8000/storage/images/${item.image}`} alt="itemimage" className='h-18 w-18'/>
                                </td>
                                <td className="px-6 py-4">
                                    {handleCategoryFind(item.category_id)}
                                </td>
                                <td className="px-6 py-4">
                                    {formatDate(item.created_at!)}
                                </td>
                                <td className="px-2 py-4 flex justify-center items-center gap-2 h-26">
                          ...
                          <tr className="bg-white h-14 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 even:bg-blue-100 even:border-y-[1px] even:border-blue-700">
                            <td colSpan={6} className='w-full'> <div className='flex justify-center items-center gap-3'><div className='relative'><img src='/icons8-box-important.gif' alt='nodata' className='h-6 absolute w-6 bottom-[-6px] right-[-6px]'/><BsDatabaseFillExclamation className="text-3xl text-blue-700"/></div>No Data Saved </div></td>
                          </tr> )
                      :
                      <tr className="bg-white h-14 dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 even:bg-blue-100 even:border-y-[1px] even:border-blue-700">
                        <td colSpan={6} className='w-full'> <div className='flex justify-center items-center'><img src='/loadingc.svg' alt='loading' className='h-10'/>Data Fetching... </div></td>
                      </tr>
                      }
                    </tbody>
                </table>
                <nav className="flex items-center flex-column flex-wrap md:flex-row justify-between pt-4" aria-label="Table navigation">
                    ...
                    {itemPagination.map((item : PAGINATION, index : number) => (
                          <li key={index} className={`${item.active ? 'bg-cyan-600 text-white' : null} flex items-center justify-center font-semibold px-3 h-8 ms-0 leading-tight text-gray-500 bg-white border border-blue-600 hover:bg-blue-900 hover:text-white dark:hover:bg-blue-900 dark:hover:text-white  dark:border-gray-700 dark:text-gray-400  cursor-pointer ${index === 0 ? 'first:rounded-l-xl' : ''} ${index === itemPagination.length - 1 ? 'last:rounded-r-xl' : ''} transition-all duration-200 delay-75`} dangerouslySetInnerHTML={{__html: item.label}} onClick={() => {handleFetchData(item.url, ''); handleSelectDelAll()}}/>
                        ))}
                    </ul>
                </nav>
            </div>
